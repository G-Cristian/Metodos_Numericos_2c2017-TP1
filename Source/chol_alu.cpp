...
// Recibe una Matrix A y devuelve un booleano. 
// Devuelve true si y solo si la Matrix es simétrica. 
// Tratar de minimizar la cantidad de comparaciones. 
bool esSimetrica(const Matrix& A) {
  if (!esCuadrada(A)) throw runtime_error("Matrix no SDP."); //solo me importan estas
  int N = filas(A);
  for(int i=0; i<N; i++){ 
...
// Recibe una Matrix A y debe devolver una nueva Matrix con la L de cholesky.
// Debe chequear que la Matrix sea cuadrada, simétrica, y luego 
...
Matrix cholesky(const Matrix &A) {
	if (!esCuadrada(A)) throw runtime_error("Matrix no SDP.");
	if (!esSimetrica(A)) throw runtime_error("Matrix no SDP.");
	int N = filas(A);
	...
	for(int j=0; j<N; j++) {
		...
		if (arg<=0) throw runtime_error("Matrix no SDP.");
		L[j][j]= sqrt(arg);
...
// Toma una Matrix A y devuelve una nueva Matrix con 
// el resultad de hacer A * A'. 
// Tratar de minimizar la cantidad de operaciones. 
// Considerar que (A * A')' = A'' * A' = A * A'
Matrix multiplicarPorTraspuesta(const Matrix& A) {
	Matrix R = Matrix(filas(A), vector<double>(filas(A), 0));
	int N = filas(A);
	int M = columnas(A);	
	for( int i=0; i<N; i++){
		for( int j=0; j<N; j++){
...
void test1() {
  ...
  Matrix L1 = cholesky(A1_test);
  assert(L1==L1_test);
  Matrix A1 = multiplicarPorTraspuesta(L1);
  assert(A1==A1_test);
}
...
void test2() {
  ...
  Matrix L2 = cholesky(A2_test);
  assert(L2==L2_test);
  Matrix A2 = multiplicarPorTraspuesta(L2);
  assert(A2==A2_test);
}

void test3() {
  // Se espera recibir una excepción porque la Matrix no es simétrica. 
  Matrix A3_test = {
    {0.066076,   0.181880},
    {1.181880,   0.624953}
  };
  try {
    Matrix L3 = cholesky(A3_test);
    assert(false);
  } catch (exception& e) {
    assert(strcmp(e.what(),"Matrix no SDP.")==0);
    cout << "La Matrix 3 no es SDP.\n";
  }
}

void test4() {
  // Se espera recibir una excepción porque la Matrix no es definida positiva. 
  Matrix A4_test = {
    {-0.066076,  0.181880},
    {0.181880,   0.624953}
  };
  try {
    Matrix L4 = cholesky(A4_test);
    assert(false);
  } catch (exception& e) {
    assert(strcmp(e.what(),"Matrix no SDP.")==0);
    cout << "La Matrix 4 no es SDP.\n";
  }
}

void testPerformance() {
  Matrix A = randomMatriz(800);
  clock_t start = clock();
  Matrix T = multiplicarPorTraspuesta(A);
  clock_t end = clock();
